...
model=pk.load(open(f'C:/Users/lekhn/model.pkl','rb'))

st.header("Car Price Prediction ML model")

cars_data=pd.read_csv(f"D:/pythonprojects/CarPricePredictionML/Cardetails.csv")

#Extract the name or element from the data to classify it easily
def get_brand_name(car_name):
    car_name=car_name.split(' ')[0]#index
...
if st.button("Predict"):
   ...
   # st.write(input_data_model)
    ...
    if car_price<=0:
        st.markdown("The type of car doest exist")
    else:
